            ...
            <div className="flex-1 overflow-y-auto px-4 pt-2 pb-6">
                ...
                <div className="bg-purple-50 border border-purple-100 rounded-2xl p-3 mb-3">
                    <p className="text-[11px] text-purple-600 font-semibold leading-relaxed">
                        📌 Ces tâches s'affichent aussi dans <strong>Todo-Projet</strong> et <strong>Gantt</strong>. Toute modification se répercute partout.
                    </p>
                </div>

                {/* Guide principal - toujours visible, repliable */}
                <div className="bg-white rounded-2xl border border-amber-200 shadow-sm overflow-hidden mb-3">
                    <button
                        onClick={() => setShowGuide(!showGuide)}
                        className="w-full px-4 py-3 flex items-center justify-between bg-gradient-to-r from-amber-50 to-yellow-50"
                    >
                        <div className="flex items-center gap-2">
                            <span className="text-base">📖</span>
                            <span className="text-xs font-black text-amber-700 uppercase tracking-wider">
                                Comment créer vos tâches
                            </span>
                        </div>
                        {showGuide ? <ChevronUp size={16} className="text-amber-500" /> : <ChevronDown size={16} className="text-amber-500" />}
                    </button>

                    {showGuide && (
                        <div className="px-4 py-3 space-y-3 text-[11px] text-slate-700 leading-relaxed">
                            ...
                            <div className="border-t border-amber-100 pt-2">
                                <p className="font-black text-slate-800 mb-1">Comment créer vos tâches pour atteindre cet objectif :</p>
                            </div>
                ...
                {/* Aide IA */}
                <div className="bg-white rounded-2xl border-2 border-indigo-200 shadow-sm overflow-hidden mb-3">
                    <button
                        onClick={() => { setShowAIHelp(!showAIHelp); setAiResponse(null); }}
