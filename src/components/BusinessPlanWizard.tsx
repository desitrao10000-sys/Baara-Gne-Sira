...
export default function BusinessPlanWizard({ initialData, onComplete, onBack }: BusinessPlanWizardProps) {
    ...
    const handleNext = () => { setDirection("forward"); if (isCAStep) setMode("review"); else setStep(step + 1); };
    const handlePrev = () => { setDirection("backward"); if (mode === "review") setMode("wizard"); else if (step > 0) setStep(step - 1); };
                ...
                <div key={step} className={`flex-1 flex flex-col p-5 overflow-y-auto ${direction === "forward" ? "animate-slide-in-right" : "animate-slide-in-left"}`}>
                    ...
                    {currentStep && !isCAStep && (
                        <>
                            ...
                            <input type="text" inputMode="decimal" value={getVal(currentStep.id)} onChange={(e) => setVal(currentStep.id, e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && canGoNext()) handleNext(); }} placeholder={stepHints[currentStep.id]?.inputHint || "📝 Écris le nom puis le montant"} className="w-full p-3.5 rounded-2xl border-2 border-slate-200 bg-white text-sm font-bold text-slate-800 outline-none focus:border-vibrant-blue focus:shadow-lg focus:shadow-blue-500/20 transition-all text-center" autoFocus />

                            {/* ── BARRE D'ASSISTANCE ── */}
                            <div className="mt-4 bg-white rounded-2xl shadow-sm border-2 border-amber-200 overflow-hidden">
                                <div className="bg-amber-50 px-3 py-2 flex items-center gap-2 border-b border-amber-200">
                                    <Lightbulb size={14} className="text-primary-yellow" />
                                    <span className="text-[11px] font-black text-amber-700 uppercase tracking-wider">{stepHints[currentStep.id]?.helpTitle || "💡 Assistant"}</span>
                                </div>
                                <div className="p-3">
                                    <div className="flex gap-2">
                                        <input type="text" value={helpQuery} onChange={(e) => { setHelpQuery(e.target.value); setHelpResponse(null); }} onKeyDown={(e) => { if (e.key === "Enter") handleHelpSubmit(); }} placeholder={stepHints[currentStep.id]?.helpPlaceholder || "Pose ta question ou décris ton projet..."} className="flex-1 p-2.5 rounded-xl border-2 border-slate-200 text-xs font-semibold text-slate-800 outline-none focus:border-amber-400" />
                                        <button onClick={handleHelpSubmit} disabled={!helpQuery.trim()} className={`px-3 py-2.5 rounded-xl flex items-center justify-center transition-all ${helpQuery.trim() ? "bg-amber-400 text-white active:scale-95" : "bg-slate-100 text-slate-400"}`} aria-label="Envoyer">
                                            <Send size={16} />
                                        </button>
                                    </div>
                                    {helpLoading && (
                                        <div className="mt-2 p-2.5 rounded-xl text-[11px] font-semibold leading-snug bg-amber-50 border border-amber-200 flex items-center gap-2">
                                    ...
                                    {helpResponse && !helpLoading && (
                                        <div ref={helpResponseRef} className={`help-response-scroll mt-2 p-4 rounded-xl text-[13px] font-semibold leading-relaxed ${helpResponse.relatedField ? "bg-blue-50 border border-blue-200" : "bg-green-50 border border-green-200"}`} style={{ maxHeight: '200px', overflow: 'scroll', scrollbarWidth: 'thin', scrollbarColor: '#94a3b8 #f1f5f9' }}>
